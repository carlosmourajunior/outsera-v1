...
def test_single_producer_two_wins_appears_in_both_extremes() -> None:
    result = compute_award_intervals({"Producer A": [2000, 2002]})
...
def test_producer_with_less_than_two_distinct_wins_is_excluded() -> None:
    ...
    assert compute_award_intervals({"Producer A": [1990]}) == compute_award_intervals(
        {}
    )
    # Duas vitorias no mesmo ano contam como uma so, apos deduplicacao (DR-06).
    assert compute_award_intervals({"Producer A": [1990, 1990]}) == compute_award_intervals(
        {}
    )
...
def test_producer_with_three_wins_produces_one_pair_per_consecutive_win() -> None:
    ...
    result = compute_award_intervals({"Producer A": [1990, 1991, 2000]})
...
def test_ties_produce_multiple_elements_ordered_by_previous_win_then_producer() -> None:
    result = compute_award_intervals(
        {
            "Producer A": [1990, 1991, 2000],  # pares: interval 1 e interval 9
            "Producer B": [1985, 1986],  # interval 1, empata com o primeiro par de A
        }
    )
...
def test_tie_with_same_previous_win_orders_by_producer_name() -> None:
    result = compute_award_intervals(
        {
            "Zebra Films": [1980, 1990],
            "Apple Studio": [1980, 1990],
        }
    )
